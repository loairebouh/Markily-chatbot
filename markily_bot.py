...
class MarkilyBot:
    ...
    def generate_pdf_report(self, user_id: int, contact_id: int, chat_id: int, user_name: str = "User") -> Optional[bytes]:
        ...
        try:
            ...
            # Title and header
            lang = get_user_language(user_id)
            story.append(Paragraph(TRANSLATIONS[lang]['pdf_title'], title_style))
            story.append(Paragraph(TRANSLATIONS[lang]['pdf_subtitle'], subtitle_style))
            story.append(Spacer(1, 20))
            
            # Contact and date info
            story.append(Paragraph(f"<b>{TRANSLATIONS[lang]['pdf_contact']}</b>".format(contact_name), styles['Normal']))
            
            # Get date range
            first_date = datetime.fromisoformat(history[-1][4]).strftime("%Y-%m-%d")
            last_date = datetime.fromisoformat(history[0][4]).strftime("%Y-%m-%d")
            story.append(Paragraph(f"<b>{TRANSLATIONS[lang]['pdf_period']}</b>".format(first_date, last_date), styles['Normal']))
            story.append(Paragraph(f"<b>Generated:</b> {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles['Normal']))
            story.append(Spacer(1, 20))
            
            # Transaction table
            data = [
                [TRANSLATIONS[lang]['pdf_date'], TRANSLATIONS[lang]['pdf_description'], 
                 TRANSLATIONS[lang]['pdf_amount'], TRANSLATIONS[lang]['pdf_balance']]
            ]
            ...
            for amount, currency, transaction_type, note, created_at in reversed(history):
                date = datetime.fromisoformat(created_at).strftime("%m/%d/%Y")
                
                if transaction_type == 'lend':
                    running_balance += amount
                    total_lent += amount
                    description = f"{TRANSLATIONS[lang]['pdf_you_lent']}"
                    amount_str = f"+{amount:,.0f}"
                else:
                    running_balance -= amount
                    total_borrowed += amount
                    description = f"{TRANSLATIONS[lang]['pdf_you_borrowed']}"
                    amount_str = f"-{amount:,.0f}"
                ...
                data.append([date, description, amount_str, balance_str])
            ...
            # Summary section
            story.append(Paragraph(f"<b>{TRANSLATIONS[lang]['pdf_summary']}</b>", styles['Heading3']))
            story.append(Paragraph(f"{TRANSLATIONS[lang]['pdf_total_lent']} {total_lent:,.0f} DZD", styles['Normal']))
            story.append(Paragraph(f"{TRANSLATIONS[lang]['pdf_total_borrowed']} {total_borrowed:,.0f} DZD", styles['Normal']))
            
            # Final balance
            final_balance = running_balance
            balance_text = f"{TRANSLATIONS[lang]['pdf_net_balance']} "
            if final_balance > 0:
                balance_text += f"+{final_balance:,.0f} DZD (Contact owes you)"
                balance_color = colors.green
            elif final_balance < 0:
                balance_text += f"{final_balance:,.0f} DZD (You owe contact)"
                balance_color = colors.red
            else:
                balance_text += "0 DZD (Settled)"
            ...
            story.append(Paragraph(balance_text, final_balance_style))
            story.append(Spacer(1, 30))
            
            # Footer
            footer_text = TRANSLATIONS[lang]['pdf_footer'].format(datetime.now().strftime("%Y-%m-%d %H:%M"))
...
async def export_pdf_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pdf_bytes:
        ...
        keyboard = [[InlineKeyboardButton(t(user_id, 'back_to_menu'), callback_data="back_to_menu")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text(t(user_id, 'pdf_error'), reply_markup=reply_markup)
