    ...
    def generate_pdf_report(self, user_id: int, contact_id: int, chat_id: int, user_name: str = "User") -> Optional[bytes]:
        ...
        try:
            ...
            def process_arabic_text(text):
                """Process Arabic text for proper RTL display and font compatibility"""
                if not text or lang != 'ar':
                    return text
                try:
                    # Ensure text is properly encoded as UTF-8
                    if isinstance(text, str):
                        # Make sure the string is properly encoded
                        text = text.encode('utf-8').decode('utf-8')
                    
                    # First try python-bidi for proper RTL text handling
                    try:
                        from bidi.algorithm import get_display
                        processed_text = get_display(text)
                        
                        # If we don't have a proper Arabic font, provide transliteration as fallback
                        if not arabic_font_registered:
                            # For critical UI elements, provide English alternatives
                            arabic_to_english = {
                                'تقرير تاريخ المعاملات': 'Transaction History Report',
                                'كشف حساب الديون والمدفوعات': 'Debt & Payment Statement',
                                'جهة الاتصال:': 'Contact:',
                                'الفترة:': 'Period:',
                                ...
                                'الرصيد': 'Balance',
                                'الملخص': 'Summary',
                                'إجمالي المُقرض:': 'Total Lent:',
                                'إجمالي المُستدان:': 'Total Borrowed:',
                            ...
                            # Check if this is a key phrase that should be translated
                            for arabic, english in arabic_to_english.items():
                                if arabic in text:
                                    processed_text = text.replace(arabic, english)
                                    logger.info(f"Using English fallback for Arabic text: {arabic} -> {english}")
                                    break
                        
                        return processed_text
                        
                    except ImportError:
                        logger.warning("python-bidi not available, using basic RTL processing")
                        # Fallback: Return text as-is, but ensure proper encoding
                        return text
